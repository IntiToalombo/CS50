...
def buy():
    ...
    if request.method == "GET":
        ...
        if shares <= 0:
            return apology(f"You need buy at least 1 share from {symbol}")
        ...
        if user_cash < transaction_value:
            return apology(f"Insufficient funds to buy {shares} shares")
        update_cash = user_cash - transaction_value

        # Udpate user's cahs after buying a share
        db.execute("UPDATE users SET cash = ? WHERE id = ?", update_cash, user_id)
        date = datetime.datetime.now()

        # Insert new data in transactions TABLE
...
def new_cash():
    ...
    if request.method == "GET":
        ...
        if not new_cash:
            return apology("A Minimun Amount of 1$ Should Be Added ")
        if not cc_number:
            return apology("A Credit Card Number Should Be Typed")
        if validate_credit_card(cc_number):
            user_cash_db = db.execute("SELECT cash FROM users WHERE id = :id", id=user_id)
            user_cash = user_cash_db[0]["cash"]
            ...
            return apology("Credit Card Number Not Valid")
        return redirect("/")
...
def quote():
    ...
    if request.method == "GET":
        ...
        # Check if the Symbol existe in DB
        stock = lookup(symbol)
...
def register():
    ...
    if request.method == "GET":
        ...
        if password != confirmation:
            return apology("Password do not matches")

        hash = generate_password_hash(password)  # Generate a has for password

        try:
            new_user = db.execute("INSERT INTO users (username, hash) VALUES (?, ?)",
                                  username, hash)  # Insert username and password(hash)
        except:
            return apology(f"Username {username} already exist")
